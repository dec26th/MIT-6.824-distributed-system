...
func checkClntAppends(t *testing.T, clnt int, v string, count int) {
	...
	for j := 0; j < count; j++ {
		...
		if off < 0 {
			t.Fatalf("%v missing element %v in Append store %v", clnt, wanted, v)
		}
		off1 := strings.LastIndex(v, wanted)
		if off1 != off {
			t.Fatalf("duplicate element %v in Append store", wanted)
		}
		if off <= lastoff {
			t.Fatalf("wrong order for element %v in Append store", wanted)
		}
		lastoff = off
	}
}
...
func checkConcurrentAppends(t *testing.T, v string, counts []int) {
	...
	for i := 0; i < nclients; i++ {
		...
		for j := 0; j < counts[i]; j++ {
			...
			off := strings.Index(v, wanted)
			if off < 0 {
				t.Fatalf("%v missing element %v in Append store %v", i, wanted, v)
			}
			off1 := strings.LastIndex(v, wanted)
			if off1 != off {
				t.Fatalf("duplicate element %v in Append store", wanted)
		...
		go spawn_clients_and_wait(t, cfg, nclients, func(cli int, myck *Clerk, t *testing.T) {
			...
			for atomic.LoadInt32(&done_clients) == 0 {
				...
				if (rand.Int() % 1000) < 500 {
					DPrintf("[test] %d: client new append %v\n", cli, nv)
					...
					DPrintf("[test] %d: client new get %v\n", cli, key)
