...
func TestBackup2B(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		DPrintf("[1]Submit %d to Raft(%d) but should not be committed", random, leader1)
		cfg.rafts[leader1].Start(random)
	}
	...
	for i := 0; i < 50; i++ {
		random := rand.Int() % 2000
		DPrintf("[2]Submit %d and should be committed", random)
		cfg.one(random, 3, true)
	}
	...
	for i := 0; i < 50; i++ {
		random := rand.Int() % 2000
		DPrintf("[4]Submit %d and should be committed", random)
		cfg.one(random, 3, true)
	}
